...
SCHEMAS: dict[str, dict] = {
    ...
    "SapHana_Alerts_CL": {
        ...
        "columns": {
            ...
            "_SERVER_UTC_t": {"type": "datetime", "description": "UTC timestamp from the HANA server at collection time."},
            "_TIMESERIES_UTC_t": {"type": "datetime", "description": "UTC timestamp of the HANA monitoring timeseries data point."},
    ...
    "SapHana_BackupCatalog_CL": {
        ...
        "columns": {
            ...
            "_SERVER_UTC_t": {"type": "datetime", "description": "UTC timestamp from the HANA server at collection time."},
            "_TIMESERIES_UTC_t": {"type": "datetime", "description": "UTC timestamp of the timeseries data point for this backup entry."},
    ...
    "SapHana_HighMemoryUsageService_CL": {
        ...
        "columns": {
            "TimeGenerated": {"type": "datetime", "description": "UTC ingest timestamp."},
            "_SERVER_UTC_t": {"type": "datetime", "description": "UTC timestamp from the HANA server."},
    ...
    "SapHana_LoadHistory_CL": {
        ...
        "columns": {
            ...
            "_SERVER_UTC_t": {"type": "datetime", "description": "UTC timestamp from the HANA server."},
            "_TIMESERIES_UTC_t": {"type": "datetime", "description": "UTC timestamp of this load data point (actual measurement time)."},
            "_SERVER_LOCALTIME_t": {"type": "datetime", "description": "HANA server local time of the load data point."},
        ...
        "kql_hints": [
            "ALWAYS filter by sapsid_s: | where sapsid_s == '<sid>'",
            "Use _TIMESERIES_UTC_t for the real measurement time when trending; TimeGenerated is the collection time.",
            "CPU pressure: | where CPU_d > 80 | summarize max(CPU_d) by bin(_TIMESERIES_UTC_t, 5m), HOST_s",
    ...
    "SapHana_SystemAvailability_CL": {
        ...
        "columns": {
            "TimeGenerated": {"type": "datetime", "description": "UTC ingest timestamp. Use as primary time filter."},
            "_SERVER_UTC_t": {"type": "datetime", "description": "UTC timestamp from the HANA server."},
            "_TIMESERIES_UTC_t": {"type": "datetime", "description": "UTC timestamp of this availability event."},
    ...
    "SapHana_SystemReplication_CL": {
        ...
        "columns": {
            "TimeGenerated": {"type": "datetime", "description": "UTC ingest timestamp. Use as primary time filter."},
            "_SERVER_UTC_t": {"type": "datetime", "description": "UTC timestamp from the HANA server."},
            "_SERVER_LOCALTIME_t": {"type": "datetime", "description": "HANA server local time at collection."},
            ...
            "SHIPPED_SAVEPOINT_START_TIME_t": {"type": "datetime", "description": "Start time of the last savepoint shipped to the secondary. A stale value while log positions keep advancing means delta data shipping has stalled."},
    ...
    "SapHana_SqlProbe_CL": {
        ...
        "columns": {
            "TimeGenerated": {"type": "datetime", "description": "UTC ingest timestamp. Use as primary time filter."},
            "_LOCAL_UTC_t": {"type": "datetime", "description": "Local UTC timestamp of probe execution."},
    ...
    "SapHana_DiskFragmentation_CL": {
        ...
        "columns": {
            "TimeGenerated": {"type": "datetime", "description": "UTC ingest timestamp."},
            "_SERVER_UTC_t": {"type": "datetime", "description": "HANA server UTC timestamp at collection."},
    ...
    "SapHana_LongIdlingCursors_CL": {
        ...
        "columns": {
            ...
            "_SERVER_UTC_t": {"type": "datetime", "description": "HANA server UTC timestamp at collection."},
            "_SERVER_LOCALTIME_t": {"type": "datetime", "description": "HANA server local time at collection."},
            "_TIMESERIES_UTC_t": {"type": "datetime", "description": "UTC timestamp of this timeseries data point."},
    ...
    "SapHana_BlockedTransactions_CL": {
        ...
        "columns": {
            "TimeGenerated": {"type": "datetime", "description": "UTC ingest timestamp. Use as primary time filter."},
            "BLOCKED_TIME_t": {"type": "datetime", "description": "Time when the blocking started. Derive wait duration as _SERVER_UTC_t - BLOCKED_TIME_t (there is no duration column on this table)."},
            "_SERVER_UTC_t": {"type": "datetime", "description": "HANA server UTC timestamp at collection."},
            "_SERVER_LOCALTIME_t": {"type": "datetime", "description": "HANA server local time at collection."},
            "_TIMESERIES_UTC_t": {"type": "datetime", "description": "UTC timestamp of this timeseries data point."},
        ...
        "kql_hints": [
            ...
            "Wait duration must be computed: | extend blocked_seconds = datetime_diff('second', _SERVER_UTC_t, BLOCKED_TIME_t)",
