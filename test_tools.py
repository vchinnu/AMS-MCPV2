...
for t in schema_summary["registered_tables"]:
    print(f"  {t['name']}")
    print(f"    time_column : {t['time_column']}")
    print(f"    sid_column  : {t['sid_column']}")
    print(f"    key_columns : {t['key_columns']}")
...
assert len(hana_names) == 12, f"Expected 12 HANA tables, got {len(hana_names)}"
for n in hana_names:
    print(f"  {n}")
...
print(f"Filter ctx sample: {result['error_investigation'][0]['filter_context']}")
...
print(f"Unknown metrics     : {os_result['unknown_metrics']}")
...
assert "sapmon" not in os_result["unknown_metrics"],             "sapmon must be filtered before classification"
...
assert len(avail_result["critical_findings"]) == 0,         "No ASCS outage → no critical findings"
...
if live_result["status"] in ("success", "partial"):
    ...
    show("Query result (last 24h across all SIDs)", live_result["rows"])
